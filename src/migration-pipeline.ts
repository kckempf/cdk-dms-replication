...
export class DmsMigrationPipeline extends Construct {
  // ---------------------------------------------------------------------------
  // Static singleton helpers — declared first to satisfy member-ordering rules
  // ---------------------------------------------------------------------------

  /**
   * Returns the account-level `dms-vpc-role` IAM role, creating it once per
   * stack. DMS requires this exact role name before it can place replication
   * instances inside a VPC. Without it, replication instance creation fails
   * with an IAM error regardless of the instance's own security group or
   * subnet group configuration.
   */
  private static ensureVpcRole(stack: cdk.Stack): iam.Role {
    const existingRole = stack.node.tryFindChild('DmsVpcRole');
    if (existingRole) {
   ...
   * CloudWatch. Using a stack-level singleton prevents duplicate-role errors
   * when multiple `DmsMigrationPipeline` constructs are deployed in the same
   * stack or account.
   */
  private static ensureCloudWatchRole(stack: cdk.Stack): iam.Role {
    const existingRole = stack.node.tryFindChild('DmsCloudWatchLogsRole');
    if (existingRole) {
      return existingRole as iam.Role;
    }
  ...
  constructor(scope: Construct, id: string, props: DmsMigrationPipelineProps) {
    ...
    // -----------------------------------------------------------------------
    if (createRoles) {
      this.dmsVpcRole = DmsMigrationPipeline.ensureVpcRole(cdk.Stack.of(this));
    }
    ...
    if (enableLogs) {
      ...
      if (createRoles) {
        ...
        this.dmsCloudWatchRole = DmsMigrationPipeline.ensureCloudWatchRole(cdk.Stack.of(this));
      }
    }
